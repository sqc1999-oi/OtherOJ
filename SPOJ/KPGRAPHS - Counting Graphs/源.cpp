...
#define gcd __gcd
	...
	inline int pdt(int a, int b, int c)
	{
		return pdt(a, pdt(b, c));
	}

	inline int pdt(int a, int b, int c, int d)
	{
...
int main()
{
	...
	{
		binom[i][0] = 1;
		REP(j, i) binom[i][j + 1] = binom[i - 1][j] + binom[i - 1][j + 1];
	}
	REP_1(i, n) S[i] = pow(2, binom[i][2]);
	gao(A);
	REP_1(i, n) S[i] = pow(2, binom[i - 1][2]);
	gao(B);
	S[0] = 1;
	REP_1(i, n)
	{
	...
	{
		C1[i] = S[i] / 2;
		REP_1(j, i - 1) C1[i] -= binom[i - 1][j - 1] * C1[j] * S[i - j];
	}
	REP_1(i, n)
	{
		C[i] = C1[i];
