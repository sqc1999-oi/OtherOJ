...
int main()
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		else if (opt == 3) cout << tree.get_rank_by_value(x) << endl;
		else if (opt == 4) cout << tree.get_value_by_rank(x) << endl;
		else if (opt == 5) cout << tree.get_predecessor(x) << endl;
		else cout << tree.get_successor(x) << endl;
	}
}
